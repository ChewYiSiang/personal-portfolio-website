  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-3">
        <span className="font-mono text-[10px] tracking-[3px] uppercase text-gray-400 flex-shrink-0">
          {category}
        </span>
        ...
        <div className="flex gap-2">
          {(['←', '→'] as const).map((arrow, i) => (
            <button
              ...
              className="w-7 h-7 rounded-full glass border border-white/[0.06] text-gray-400 hover:text-cyber-cyan hover:border-cyber-cyan/30 flex items-center justify-center text-xs transition-all"
      ...
      <div ref={trackRef} className="chip-scroll-track">
        {chips.map((chip) => (
          <div key={chip.name} className="tech-chip">
            ...
            <span className="font-mono text-[9.5px] text-gray-300 text-center leading-tight relative z-10">
              {chip.name}
            </span>
          </div>
        ))}
